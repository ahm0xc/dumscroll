...
export default function PricingTableFooter() {
  const router = useRouter();

  const { paddle } = usePaddle();
  const { user, isLoaded: isUserLoaded } = useUser();
  const { theme } = useTheme();
  const { data: subscriptionStatus, isSubscribed } = useSubscriptionStatus();
  ...
  function handleCheckout() {
    ...
    paddle?.Checkout.open({
      items: [
        {
          priceId: env.NEXT_PUBLIC_PADDLE_MONTHLY_PRICE_ID,
          quantity: 1,
        },
      ],
      customData: {
      ...
      settings: {
        ...
        successUrl: "https://dumscroll.com",
        allowLogout: false,
    ...
    <footer className="mt-10 flex items-center gap-3">
      {isSubscribed ? (
        <ManageSubscriptionButtons
          updatePaymentMethodUrl={subscriptionStatus?.updatePaymentMethodUrl}
          cancelUrl={subscriptionStatus?.cancelUrl}
        />
      ) : (
        <>
          <button
            ...
            onClick={
              user
                ? handleCheckout
                : () => {
                    router.push(`/sign-in?redirect_url=${window.location.origin}/pricing`);
                  }
            }
            disabled={!isUserLoaded}
          >
            <span>Start 3 days trial</span>
          </button>
          <div className="ml-4 flex items-center gap-4">
            ...
            <p className={cn("text-lg", handWritingFont.className)}>Make it yours now</p>
          </div>
        </>
      )}
    </footer>
...
function ManageSubscriptionButtons({
  ...
  if (!updatePaymentMethodUrl || !cancelUrl) {
    return null;
  }

  return (
    <div className="flex items-center gap-5">
      <Button size="lg" variant="default" asChild type="button">
        <a href={updatePaymentMethodUrl}>Manage Subscription</a>
      </Button>
      <Button size="lg" variant="outline" asChild type="button">
        <a href={cancelUrl}>Cancel Subscription</a>
      </Button>
    </div>
  );
}
